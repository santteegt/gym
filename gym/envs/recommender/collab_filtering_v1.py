...
# class CollaborativeFilteringV1(gym.Env):
class CollaborativeFilteringV1(gym.Env):
    ...
    def _get_guided_random_item(self):
        ...
        # random item state
        random_item_id = user_items[random_item]['item_id'] if self.properties.kwargs['use_mongodb'] \
            else user_items[random_item]
        # print "random item id in guided exploration: {}".format(random_item_id)
        return random_item_id, self.__data.find_one(query=random_item_id, type="items")
